...
class Augmentation(object):
    def __init__(self, size=640, mean=MEAN, std=STD):
        self.mean = mean
        self.size = size
        self.std = std
    ...
    def __call__(self, image, label):
        ...
        x1y1x2y2 = np.concatenate((x1y1, x2y2), axis=1).clip(min=0, max=1)
        cls_ids = label[:, 0]
        im, boxes, labels = self.augment(image, x1y1x2y2, cls_ids)
        wh = boxes[:, 2:] - boxes[:, :2]
        xcyc = boxes[:, :2] + wh / 2
        xcycwh = np.concatenate((xcyc, wh), axis=1)
        input_tensor = im
        labels = np.concatenate((labels[:, np.newaxis], xcycwh), axis=1)
        return input_tensor, labels

class BaseTransform(object):
    def __init__(self, size, mean=MEAN, std=STD):
        self.size = size
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
